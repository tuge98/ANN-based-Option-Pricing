...
def hedging_func(df):
    ...
    df["VB"] = np.where(df["rank"] > 0, (np.exp(0.01) * df["VBlag"]) - df["UNDERLYING"] * (df["deltaANN"] - df["lag_deltaANN"]), df["VB"])
    ...
    df["VBBS"] = np.where(df["rank"] > 0, (np.exp(0.01) * df["VBBSlag"]) - df["UNDERLYING"] * (df["deltaANN"] - df["lag_deltaANN"]), df["VB"])
