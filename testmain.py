...
from datahandling import read_files, manipulate_rf_data, manipulate_option_data, vol_data
...
from filtering import filteringfunction
from neural_nets import MLPnetwork
from hedging_scenario import hedging_func
...
if __name__ == '__main__':
    ...
    df["bsprice"] = blackscholes.blackscholes(df["UNDERLYING"],df["STRIKE"],df["TTM"],0.01,df["3mvol"])
    df["bsdelta"] = blackscholes.bsdelta(df["UNDERLYING"],df["STRIKE"],df["TTM"],0.01,df["3mvol"])
    ...
    n_train = (int)(0.8 * n)
    train = df1[0:n_train]
    X_train = train[['Moneyness', 'TTM','1mvol','3mvol','60dvol', 'garch']].values
    y_train = train['CALL_PRICE2'].values
    test = df1[n_train+1:n]
    X_test = test[['Moneyness', 'TTM','1mvol','3mvol','60dvol','garch']].values
    y_test = test['CALL_PRICE2'].values
    model = MLPnetwork(num_epochs=10,batch=64,nodes=120,X_train=X_train,y_train=y_train,X_test=X_test, y_test = y_test)
